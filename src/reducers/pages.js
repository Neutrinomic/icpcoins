...
function tokenPage(dispatch, getState) {
    ...
    const getPairData = () => {
        ...
        let pids = unique([1, 6, ...getPairIds(s.config, tid, 3), ...getPairIds(s.config, tid, 0)]); // against ICP and USD
    ...
    const getTokenData = () => {
        ...
        dispatch(fetchTokens({ interval: 't1d', ids: unique([0, 1, 2, 3, tid]), back: period }));
